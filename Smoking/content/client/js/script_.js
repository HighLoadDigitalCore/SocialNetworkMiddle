...
function newsFavorite() {
    $('.stream .icon-star').parent().click(function () {
        ...
        $.post('/Master/ru/Lenta/ToggleFavorite', { id: $(this).attr('arg') }, function (d) {
            ...
            var num = parseInt($.trim(a.parent().parent().find('.icon-fav').parent().text()));
            if (a.hasClass('-active')) {
                num++;
            } else {
                num--;
            }
            a.parent().parent().find('.icon-fav').parent().html('<i class="icon-fav"></i>&nbsp;' + num);
        });
        return false;
    });
}
...
function messageSend() {
    ...
    $.post('/Master/ru/Account/SendLetter', { uid: $('#PrivateUser').val(), msg: $('#PrivateMessage').val() }, function(d) {
        if (d.length) {
            ...
            e.html('Сообщение успешно отправлено.');
            e.css('color', 'green');
        }
    });
}
